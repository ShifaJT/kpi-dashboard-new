...
st.markdown("""
    <style>
        .css-1v0mbdj.ef3psqc12 {visibility: hidden;} /* hide the Streamlit menu */
        .css-164nlkn.egzxvld1 {visibility: hidden;} /* hide the Made with Streamlit */
    </style>
...
if emp_id and selected_week:
    ...
    if not week_data.empty:
        ...
        col2.metric("Auto On Time (min)", round(week_data["Auto On"].sum(), 2))
        col3.metric("AHT (sec)", round(week_data["AHT"].mean(), 2))

        col4, col5, col6 = st.columns(3)
        col4.metric("Wrap Time (sec)", round(week_data["Wrap"].mean(), 2))
        col5.metric("Hold Time (sec)", round(week_data["Hold"].mean(), 2))
        col6.metric("Productivity Score", round(week_data["Score"].mean(), 2))
    else:
